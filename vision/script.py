import os
import math

cuda_visible_devices = input()  # Set your desired GPU index here

os.environ["CUDA_VISIBLE_DEVICES"] = cuda_visible_devices

dataset = "resisc45"
bs = 32
model='vit-base'
if model == 'vit-large':
    lr = 1e-3
else:
    lr = 2e-3

method='lora'
#method='oh'

init='True'
#init='pissa_niter_4'

epoch = 20

if method == 'lora':
    ratio = 0
else:
    ratio = 1-20/epoch

for seed in [1]:
    #for r in [16,64,256,512]:
    for r in [1]:
        for scale in [8]:
            alpha = int(math.sqrt(r) * scale)
            if method == 'lora':
                output_dir = f"./cub200_results/{dataset}_{model}_lora_epoch{epoch}_bs{bs}_init{init}_lr{lr}_alpha{scale}_r{r}/"
            elif method == 'oh':
                output_dir = f"./cub200_results/{dataset}_{model}_oh_epoch{epoch}_bs{bs}_lr{lr}_ratio{ratio}_alpha{scale}_r{r}/"
            #output_dir = f"./cub200_results/test/"
            cmd = (
                f"python finetune.py "
                f"--eval_strategy epoch "
                f"--save_strategy no "
                f"--gradient_accumulation_steps 1 "
                f"--logging_steps 10 "
                f"--label_names labels "
                f"--remove_unused_columns False "
                f"--per_device_train_batch_size {bs} "
                f"--per_device_eval_batch_size 256 "
                f"--seed {seed} "
                f"--num_train_epochs {epoch} "
                f"--lora_rank {r} "
                f"--output_dir {output_dir} "
                f"--model_name {model} "
                f"--finetuning_method {method} "
                f"--dataset_name {dataset} "
                f"--clf_learning_rate {lr} "
                f"--other_learning_rate {lr} "
                f"--warmup_ratio 0.1 "
                f"--weight_decay 0.01 "
                f"--lora_alpha {alpha} "
                f"--lora_init {init} "
                f"--prepare_ratio {ratio} "
            )
            os.system(cmd)

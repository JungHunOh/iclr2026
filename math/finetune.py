...
def train():
    ...
    if lora_args.adapter_name == 'boft':
    ...
    elif lora_args.adapter_name == 'lora':
        config = LoraConfig(
            r=lora_args.lora_r,
    ...
    elif lora_args.adapter_name == 'vera':
        config = VeraConfig(
                r=lora_args.rank,
    ...
    if lora_args.adapter_name != 'svft':
        ...
        print(f"Target Modules: {lora_args.target_modules}")
        assign_svft_layer = partial(LinearWithSVFT, 
                                    off_diag=lora_args.off_diag, 
                                    pattern=lora_args.pattern, 
                                    rank=lora_args.rank, 
                                    fill_orthonormal=lora_args.fill_orthonormal)
        
        create_and_replace_modules(model, get_target_modules_list(model, lora_args.target_modules), assign_svft_layer)
